...
const commentCtrl = {
    createComment: async (req, res) => {
        try {
            ...
            if(reply){
                ...
                if(!cm) return res.status(400).json({msg: "Данный комментарий не найден."})
            }
    ...
    likeComment: async (req, res) => {
        try {
            ...
            if(comment.length > 0) return res.status(400).json({msg: "Пост оценен!."})

            await Comments.findOneAndUpdate({_id: req.params.id}, {
                $push: {likes: req.user._id}
            }, {new: true})

            res.json({msg: 'Комментарий оценен!'})
